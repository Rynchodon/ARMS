	...
	public static class Showoff
	{
		...
		private class sortableLastSeen : IComparable<sortableLastSeen>
		{
			...
			public StringBuilder ToStringBuilder(int count)
			{
				...
				builder.Append(tab);
				if (relations.HasFlagFast(IMyCubeBlockExtensions.Relations.Faction) || relations.HasFlagFast(IMyCubeBlockExtensions.Relations.Owner))
				{
					builder.Append(seen.Entity.getBestName());
					builder.Append(tab);
				}
				else
				...
				builder.Append(':');
				builder.Append(Math.Round(predictedPos.X, 2));
				builder.Append(':');
				builder.Append(Math.Round(predictedPos.Y, 2));
				builder.Append(':');
				builder.Append(Math.Round(predictedPos.Z, 2));
				builder.Append(":\n");
