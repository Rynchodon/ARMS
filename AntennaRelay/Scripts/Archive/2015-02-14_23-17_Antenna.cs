...
namespace Rynchodon.AntennaRelay
{
	...
	public class Antenna : MyGameLogicComponent, Broadcaster
	{
		...
		private void sendTransmission(LinkedList<Antenna> canSeeMe, LinkedList<Antenna> canSendTo)
		{
			...
			{
				log("failed to get a lock", "sendTransmission()", Logger.severity.DEBUG);
				return;
			}
			try
			{
			...
			finally
			{
				...
				log("function took: " + (DateTime.UtcNow - start).TotalMilliseconds + " millis", "sendTransmission()", Logger.severity.TRACE);
			}
		}
