...
namespace Rynchodon.AntennaRelay
{
	...
	public class LastSeen //: Transmission
	{
		...
		public LastSeen(IMyEntity entity, string knownName = null, bool EntityHasRadar = false)
		{
			this.Entity = entity;
			this.LastSeenAt = DateTime.UtcNow;
			this.LastKnownPosition = entity.GetPosition();
		...
		/// <summary>
		/// gives a 10s bonus when EntityHasRadar
		/// </summary>
		/// <param name="other"></param>
		/// <returns></returns>
		public bool isNewerThan(LastSeen other)
		{
			if (EntityHasRadar == other.EntityHasRadar)
				return LastSeenAt.CompareTo(other.LastSeenAt) > 0; 

			DateTime myValue = LastSeenAt;
			if (EntityHasRadar)
				myValue.AddSeconds(10);
			else
				myValue.AddSeconds(-10);
			return myValue.CompareTo(other.LastSeenAt) > 0;
		}
