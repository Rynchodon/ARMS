		...
		public static void doShowoff(IMyCubeBlock showoff, IEnumerator<LastSeen> toDisplay)
		{
			...
			while(toDisplay.MoveNext())
			{
				...
				if (relations.HasFlagFast(IMyCubeBlockExtensions.Relations.Enemies))
				{
					...
					displayText.Append(':');
					displayText.Append(gridPosition.X);
					displayText.Append(':');
					displayText.Append(gridPosition.Y);
					displayText.Append(':');
					displayText.Append(gridPosition.Z);
					displayText.Append(':');
					displayText.Append('\n');
				}
