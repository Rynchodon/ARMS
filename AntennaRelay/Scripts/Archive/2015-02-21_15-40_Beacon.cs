...
namespace Rynchodon.AntennaRelay
{
	...
	public class Beacon : UpdateEnforcer
	{
		...
		private const string SubTypeSearch = "Radar";
		...
		protected override void DelayedInit()
		{
			...
			if (CubeBlock.BlockDefinition.SubtypeName.Contains(SubTypeSearch))
			{
				isRadar = true;
		...
		public override void UpdateAfterSimulation100()
		{
			...
			try
			{
				...
				LastSeen self = new LastSeen(CubeBlock.CubeGrid, null, isRadar);
				...
				foreach (IMyEntity ent in allEntitiesInWorld)
					if (ent is IMyCubeGrid)
					{
						...
						if ((ent as IMyCubeGrid).IsStatic)
							log("static grid has mass of " + (ent as IMyCubeGrid).Physics.Mass+", volume of "+(ent as IMyCubeGrid).LocalAABB.Volume());
