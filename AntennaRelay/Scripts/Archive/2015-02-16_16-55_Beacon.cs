			...
			try
			{
				...
				foreach (IMyEntity ent in allEntitiesInWorld)
					if (ent is IMyCubeGrid || ent is IMyPlayer)
					{
						...
						IMyCubeGrid asGrid = ent as IMyCubeGrid;
						if (asGrid != null)
							log("radar found a grid: " + asGrid.DisplayName, "UpdateAfterSimulation100()", Logger.severity.TRACE);
						else
						{
							IMyPlayer asPlayer = ent as IMyPlayer;
							if (asPlayer != null)
								log("radar found a player: " + asPlayer.DisplayName, "UpdateAfterSimulation100()", Logger.severity.TRACE);
						}
