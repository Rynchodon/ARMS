...
namespace Rynchodon.AntennaRelay
{
	public abstract class Receiver : UpdateEnforcer
	{
		...
		public static void sendToAttached(IMyCubeBlock sender, ICollection<LastSeen> toSend, ICollection<Receiver> additional = null)
		{
			...
			foreach (RadioAntenna radioAnt in RadioAntenna.registry)
				if ((additional != null && additional.Contains(radioAnt)) || sender.canSendTo(radioAnt.CubeBlock, true))
					foreach (LastSeen seen in toSend)
						radioAnt.receive(seen);

			// to laser antenna
			foreach (LaserAntenna laserAnt in LaserAntenna.registry)
				if ((additional != null && additional.Contains(laserAnt)) || sender.canSendTo(laserAnt.CubeBlock, true))
					foreach (LastSeen seen in toSend)
						laserAnt.receive(seen);

			// to remote control
			foreach (RemoteControl remote in RemoteControl.registry.Values)
				if ((additional != null && additional.Contains(remote)) || sender.canSendTo(remote.CubeBlock, true))
					foreach (LastSeen seen in toSend)
						remote.receive(seen);
		}
		...
		/// </summary>
		/// <param name="additional">also sends to this collection, without testing</param>
		public static void sendToAttached(IMyCubeBlock sender, ICollection<Message> toSend, ICollection<Receiver> additional = null)
		{
