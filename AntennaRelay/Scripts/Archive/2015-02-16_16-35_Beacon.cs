...
namespace Rynchodon.AntennaRelay
{
	...
	public class Beacon : UpdateEnforcer
	{
		...
		public override void Init(MyObjectBuilder_EntityBase objectBuilder)
		{
			...
			switch (CubeBlock.BlockDefinition.SubtypeName)
			{
				...
				case SubTypeRadarSmall:
					isRadar = true;
					log("init as radar: " + CubeBlock.BlockDefinition.SubtypeName, "Init()", Logger.severity.DEBUG);
					break;
				default:
					log("init as beacon: " + CubeBlock.BlockDefinition.SubtypeName, "Init()", Logger.severity.DEBUG);
					break;
			}
		...
		public override void UpdateAfterSimulation100()
		{
			...
			try
			{
				...
				foreach (IMyEntity ent in allEntitiesInWorld)
					if (ent is IMyCubeGrid || ent is IMyPlayer)
					{
						...
						log("radar found an entity: " + Entity.DisplayName, "UpdateAfterSimulation100()", Logger.severity.TRACE);
		...
		{
			MyAPIGateway.Utilities.ShowNotification("Beacon: " + CubeBlock.BlockDefinition.SubtypeName, 3000, MyFontEnum.DarkBlue);


			if (myLogger == null)
			{
				myLogger = new Logger(CubeBlock.CubeGrid.DisplayName, "Beacon");
				MyAPIGateway.Utilities.ShowNotification("creating logger: " + CubeBlock.BlockDefinition.SubtypeName, 3000, MyFontEnum.DarkBlue);


			}
			MyAPIGateway.Utilities.ShowNotification("logging: " + CubeBlock.BlockDefinition.SubtypeName, 3000, MyFontEnum.DarkBlue);
