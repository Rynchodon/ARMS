...
namespace Rynchodon.AntennaRelay
{
	...
	public class Antenna : Receiver //, Broadcaster
	{
		...
		public override void UpdateAfterSimulation100()
		{
			...
			try
			{
				...
				foreach (KeyValuePair<IMyCubeBlock, RemoteControl> pair in RemoteControl.registry)
					if (AttachedGrids.isGridAttached(CubeBlock.CubeGrid, pair.Key.CubeGrid))
				...
				if (canSeeMe.Count != 0 || canSendTo.Count != 0)
					sendTransmission(canSeeMe, canSendTo, attachedRemote);
			}
			catch (Exception e)
		...
		private void sendTransmission(LinkedList<Antenna> canSeeMe, LinkedList<Antenna> canSendTo, LinkedList<RemoteControl> canSendToRemote)
		{
			...
			if (canSendTo.Count == 0 && canSendToRemote.Count == 0) // no friends in range
			//{
