		...
		public override void UpdateAfterSimulation()
		{
			...
			try
			{
				...
				if (myTurretBase.Elevation < limit_el_min)
				{
					...
					myLogger.debugLog("limit_el_min = " + limit_el_min, "UpdateAfterSimulation100()", Logger.severity.TRACE, myCubeBlock.DisplayNameText);
				}
				else if (myTurretBase.Elevation > limit_el_max)
				{
					limit_el_max = myTurretBase.Elevation;
					myLogger.debugLog("limit_el_max = " + limit_el_max, "UpdateAfterSimulation100()", Logger.severity.TRACE, myCubeBlock.DisplayNameText);

				}
				if (myTurretBase.Azimuth < limit_az_min)
				{
					limit_az_min = myTurretBase.Azimuth;
					myLogger.debugLog("limit_az_min = " + limit_az_min, "UpdateAfterSimulation100()", Logger.severity.TRACE, myCubeBlock.DisplayNameText);

				}
				else if (myTurretBase.Azimuth > limit_az_max)
				{
					limit_az_max = myTurretBase.Azimuth;
					myLogger.debugLog("limit_az_max = " + limit_az_max, "UpdateAfterSimulation100()", Logger.severity.TRACE, myCubeBlock.DisplayNameText);
				}
