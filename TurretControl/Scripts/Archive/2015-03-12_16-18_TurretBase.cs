...
namespace Rynchodon.Autopilot.Turret
{
	...
	public class TurretBase : UpdateEnforcer
	{
		...
		private void getValidTargets()
		{
			...
			validTarget_player = new List<IMyEntity>();
			...
			foreach (IMyEntity entity in entitiesInRange)
			{
				...
				{
					myLogger.debugLog("found a player: " + entity.getBestName(), "getValidTargets()");
					if (targetPlayers)
			...
			myLogger.debugLog("target counts = " + validTarget_missile.Count + ", " + validTarget_meteor.Count + ", " + validTarget_player.Count + ", " + validTarget_block.Count, "getValidTargets()");
