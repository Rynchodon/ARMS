	...
	public class TurretBase : UpdateEnforcer
	{
		...
		private void getValidTargets()
		{
			...
			foreach (IMyEntity entity in entitiesInRange)
			{
				...
				{
					myLogger.debugLog("found a character: " + entity.getBestName(), "getValidTargets()");
					if (targetCharacters)
			...
			//{
			//	myLogger.debugLog("targeting characters currently " + TrackerCharacter.registry.Count + " exist", "getPriorities()");
			//	targetGetters.Add(turretPosition => { return closestInRange(turretPosition, TrackerCharacter.registry); });
			//}
		}
