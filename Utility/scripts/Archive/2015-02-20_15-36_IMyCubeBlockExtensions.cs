...
namespace Rynchodon
{
	...
	{
		[Flags]
		public enum Relations : byte { None = 0, Enemies = 1, Neutral = 2, Faction = 4, Owner = 8 }
		...
		{
			if (rel.HasFlagFast(Relations.Enemies))
				return false;
		...
		{
			foreach (Relations flag in new Relations[] { Relations.Enemies, Relations.Neutral, Relations.Faction, Relations.Owner })
		...
		{ return toIsFriendly(block.getRelationsTo(target, Relations.Enemies | Relations.Neutral)); }
		...
		{ return toIsHostile(block.getRelationsTo(target, Relations.Enemies)); }
