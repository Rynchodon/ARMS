			...
			try
			{
				if (logWriter == null)
				{
					if (MyAPIGateway.Utilities == null)
						return; // cannot log
					try { logWriter = MyAPIGateway.Utilities.WriteFileInLocalStorage("log.txt", typeof(Logger)); }// was getting a NullReferenceException in ..cctor(), so we delay
					catch { logWriter = MyAPIGateway.Utilities.WriteFileInLocalStorage("log-1.txt", typeof(Logger)); }
				}
