...
namespace Rynchodon
{
	...
	public abstract class UpdateEnforcer : MyGameLogicComponent
	{
		...
		{
			(Entity as Sandbox.ModAPI.IMyCubeGrid).DisplayName += '1';
			if (IsInitialized)
				return;
			updateCount++;
			if (updateCount < 100)
		...
		{
			(Entity as Sandbox.ModAPI.IMyCubeGrid).DisplayName += '2';
			if (IsInitialized)
				return;
			updateCount += 10;
			if (updateCount < 100)
		...
		{
			(Entity as Sandbox.ModAPI.IMyCubeGrid).DisplayName += '3';
			if (IsInitialized)
				return;
