	...
	public abstract class UpdateEnforcer : MyGameLogicComponent
	{
		...
		protected MyEntityUpdateEnum EnforcedUpdate
		{
			...
			set
			{
				...
				//prependToRC(Entity.NeedsUpdate.ToString());
			}
		}
		...
		public override sealed void Init(MyObjectBuilder_EntityBase objectBuilder)
		{
			...
			EnforcedUpdate = MyEntityUpdateEnum.EACH_10TH_FRAME;
		}
		...
		private void prependToRC(char c)
		{
			Sandbox.ModAPI.Ingame.IMyTerminalBlock term = (Entity as Sandbox.ModAPI.Ingame.IMyTerminalBlock);
			if (Entity is Sandbox.ModAPI.Ingame.IMyRemoteControl)
			{
				term.SetCustomName(c + term.DisplayNameText);
			}
		}

		private void prependToRC(string s)
		{
			Sandbox.ModAPI.Ingame.IMyTerminalBlock term = (Entity as Sandbox.ModAPI.Ingame.IMyTerminalBlock);
			if (Entity is Sandbox.ModAPI.Ingame.IMyRemoteControl)
			{
				term.SetCustomName(s + term.DisplayNameText);
			}
		}
