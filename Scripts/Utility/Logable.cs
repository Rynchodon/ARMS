...
namespace Rynchodon.Utility
{
	...
	public static class LogableFrom
	{
		public static Logable Pseudo(PseudoBlock pseudo, string SecondaryState = null) {
			return new Logable(pseudo.Grid.nameWithId(), pseudo.DisplayName, SecondaryState, pseudo.Block != null);
		}
	}
	...
	public struct Logable
	{
		...
		public Logable(string Context, string PrimaryState = null, string SecondaryState = null, bool Disabled = false)
		{
		...
		public Logable(IMyEntity entity, bool Disabled = false)
		{
			this.Disabled = Disabled;
			if (entity == null)
			{
		...
		{
			if (condition)
				Logger.TraceLog(toLog, level, Context, PrimaryState, SecondaryState, condition && !Disabled, filePath, member, lineNumber);
		}
		...
		{
			if (condition)
				Logger.DebugLog(toLog, level, Context, PrimaryState, SecondaryState, condition && !Disabled, filePath, member, lineNumber);
		}
		...
		{
			if (condition)
				Logger.ProfileLog(toLog, level, Context, PrimaryState, SecondaryState, condition && !Disabled, filePath, member, lineNumber);
		}
		...
		{
			Logger.TraceLog("entered member", Logger.severity.TRACE, Context, PrimaryState, SecondaryState, !Disabled, filePath, member, lineNumber);
		}
		...
		{
			Logger.TraceLog("leaving member", Logger.severity.TRACE, Context, PrimaryState, SecondaryState, !Disabled, filePath, member, lineNumber);
		}
