...
namespace Rynchodon.BlockCommunication
{
	...
	public static class MessageParser
	{
		...
		public static Message getFromName(IMyTerminalBlock sender)
		{
			...
			{
				log("nothing to send for " + name, "getFromName()", Logger.severity.DEBUG);
				return null;
			}
			log("start = " + start + ", length = " + length, "getFromName()", Logger.severity.TRACE);
			...
			return new Message((sender as IMyCubeBlock).CubeGrid.DisplayName, NameNew, sent[0], sent[1], message);
		}
		...
		public static bool writeToName(IMyTerminalBlock recipient, Message message)
		{
			...
			recipient.SetCustomName(recipient.CustomName + startOfReceive + message.SourceGrid + separator + message.SourceBlock + separator + message.Transmission + endOfReceive);
			return true;
		}
