...
namespace Rynchodon.BlockCommunication
{
	...
	public static class MessageParser
	{
		...
		public static Message getFromName(IMyTerminalBlock sender)
		{
			...
			if (start == 0 || length < 1)
			{
				log("nothing to send for " + sender.DisplayNameText, "getFromName()", Logger.severity.TRACE);
			...
			if (sent.Length < 3)
			{
				log("cannot send while split has length "+sent.Length+", for " + sender.DisplayNameText, "getFromName()", Logger.severity.TRACE);
